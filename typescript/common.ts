...
import { readableStreamToString, SentdmError, parseIfJson } from "./error";
            ...
            if (e instanceof AxiosError && e.isAxiosError) {
                ...
                try {
                    const responseBody =
                        ...
                        : e.response?.data
                    throw new SentdmError(e, parseIfJson(responseBody), e.response?.headers)
                } catch (innerError) {
                    if (innerError instanceof ReferenceError) {
                        ...
                        throw new SentdmError(e, e.response?.data, e.response?.headers)
                    }
                    if (innerError instanceof SentdmError) {
                        // Got "SentdmError" from the above try block
                        throw innerError;
                    }
