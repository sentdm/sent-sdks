...
@interface SCustomersApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation SCustomersApi

NSString* kSCustomersApiErrorDomain = @"SCustomersApiErrorDomain";
NSInteger kSCustomersApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createCustomerWithBody: (NSString*) body
    ...
    if (body == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kSCustomersApiErrorDomain code:kSCustomersApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/customers"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"SCustomer*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SCustomer*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) deleteCustomerWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/customers/{id}"];
...
-(NSURLSessionTask*) getAllCustomersWithPage: (NSNumber*) page
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"NSArray<SCustomer>*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((NSArray<SCustomer>*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) updateCustomerWithId: (NSString*) _id
    ...
    if (customersUpdateCustomerRequest == nil) {
        NSParameterAssert(customersUpdateCustomerRequest);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"customersUpdateCustomerRequest"] };
            NSError* error = [NSError errorWithDomain:kSCustomersApiErrorDomain code:kSCustomersApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableDictionary *formParams = [[NSMutableDictionary alloc] init];
    NSMutableDictionary *localVarFiles = [[NSMutableDictionary alloc] init];
    bodyParam = customersUpdateCustomerRequest;
